...
from BLEval.computeSTREAMLINE import STREAMLINE_multicore
...
class BLEval(object):
    ...
    def computeSTREAMLINE_global(self, directed = True, verbose=True):

        '''
        For each algorithm-dataset combination, this function computes the global (per-network) topologic properties and saves the results to csv files

        :returns:ns
        '''
        if(directed):
            self.input_settings.algorithms = [['GRNBOOST2', {'should_run': True}], ['SINCERITIES', {'should_run': True}]]
        STREAMLINE_multicore(self.input_settings, mode="global", directed = directed, saveraw=True, verbose=verbose)

        return

    def computeSTREAMLINE_local(self, directed = True, verbose=True):

        '''
        For each algorithm-dataset combination, this function computes the local (per-node) topologic properties and saves the results to csv files

        :returns:ns
        '''
        if(directed):
            self.input_settings.algorithms = [['GRNBOOST2', {'should_run': True}], ['SINCERITIES', {'should_run': True}]]
        STREAMLINE_multicore(self.input_settings, mode="local", directed = directed, saveraw=True, verbose=verbose)
    ...
    def computeEarlyPrec(self, directed = True):
        ...
        for algo in tqdm(self.input_settings.algorithms, unit = " Algorithms"):
            if algo[1]['should_run'] == True:
                Eprec[algo[0]] = EarlyPrec(self, algo[0], directed=directed)
