...
def EarlyPrec(evalObject, algorithmName, TFEdges = False, directed=True):
    '''
    ...
    for dataset in tqdm(evalObject.input_settings.datasets):
        ...
        if not directed:
            trueEdgesDF.loc[trueEdgesDF['Gene1'] > trueEdgesDF['Gene2'], trueEdgesDF.columns[[0,1]]] = trueEdgesDF.loc[trueEdgesDF['Gene1'] > trueEdgesDF['Gene2'], trueEdgesDF.columns[[1,0]]].values
        ...
        if not directed:
            predDF.loc[predDF['Gene1'] > predDF['Gene2'], predDF.columns] = predDF.loc[predDF['Gene1'] > predDF['Gene2'], predDF.columns[[1,0,2]]].values
        predDF.drop_duplicates(subset=['Gene1', 'Gene2'], keep = 'first', inplace=True)
        predDF = predDF[predDF['Gene1'].isin(trueEdgesDF['Gene1']) | predDF['Gene1'].isin(trueEdgesDF['Gene2'])]
        predDF = predDF[predDF['Gene2'].isin(trueEdgesDF['Gene1']) | predDF['Gene2'].isin(trueEdgesDF['Gene2'])]
