...
def get_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('-tloc','--STREAMLINE_local', action="store_true", default=False,
      help="Analyse the local network topology (per node).")

    parser.add_argument('-tglob','--STREAMLINE_global', action="store_true", default=False,
      help="Analyse the global network topology (per graph).")
...
def main():
    ...
    if (opts.auc):
        ...
        AUPRC, AUROC = evalSummarizer.computeAUC(directed = opts.directed)
        AUPRC.to_csv(outDir + 'AUPRC-' + ("directed" if opts.directed else "undirected") + '.csv')
        AUROC.to_csv(outDir + 'AUROC-' + ("directed" if opts.directed else "undirected") + '.csv')
    ...
    if (opts.epr):
        ...
        ePRDF = evalSummarizer.computeEarlyPrec(directed = opts.directed)
        ePRDF.to_csv(outDir + 'EPr-' + ("directed" if opts.directed else "undirected") + '.csv')
   ...
   # Compute edge ranked list using the borda method
    ...
    # Compute global topological properties (per graph)
    if (opts.STREAMLINE_global):
        print('\n\nPerforming global STREAMLINE analysis')
        evalSummarizer.computeSTREAMLINE_global(directed = opts.directed, verbose=opts.verbose)
    # Compute local topological properties (per node)
    if (opts.STREAMLINE_local):
        print('\n\nPerforming local STREAMLINE analysis')
        evalSummarizer.computeSTREAMLINE_local(directed = opts.directed, verbose=opts.verbose)
